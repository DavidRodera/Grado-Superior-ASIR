---
title:   "UTILIDADES"

author: "David Rodera"
subject: "Fundamentos de Hardware"
date: "2024–2025"
---

# Diagnóstico, Reparación y Mantenimiento de Discos

## Badblocks

- Escanea los sectores del disco duro o SSD para encontrar áreas dañadas que el sistema de archivos ignora.

- SO: **Linux** 

- Interfaz: **CLI**

- Origen: **Nativa**

## e2fsck

- Verifica y repara los errores en ext2, ext3 y ext4.

- SO: **Linux** 

- Interfaz: **CLI**

- Origen: **Nativa**

- Opciones de lanzamiento esenciales: **-p** (Reparación automática), **-y** (Asumir "Sí"), **-f** (Forzar), **-c** (Comprobar bloques dañados), **-n** (Solo lectura) y **-v** (Modo detallado)

## sfc

- Analiza los archivos del sistema, reparandolos en caso de corrupción

- SO: **Windows** 

- Interfaz: **CLI**

- Origen: **Nativa**

# Limpieza y Optimización del Sistema

## Bleachbit

- Limpia los discos, optimiza el sistema y gestiona la privacidad.

- SO: **Linux** 

- Interfaz: **CLI**

- Origen: **De Terceros**

## Stacer

- Optimiza y monitorea el sistema

- SO: **Linux** 

- Interfaz: **GUI**

- Origen: **De Terceros**

- Funcionalidades incorporadas del entorno gráfico: Muestra un resumen del uso del CPU, memoria RAM, almacenamiento del disco duro e información del SO; además, hay un limpiador del sistema que permite escanear y eliminar archivos.

# Análisis de Espacio y Almacenamiento

## TreeSize

- Escanea visualmente el árbol de directorios para encontrar donde se ha ido el espacio en disco

- SO: **Windows** 

- Interfaz: **GUI**

- Origen: **De Terceros**

- Funcionalidades incorporadas del entorno gráfico: Muestra una estructura de árbol de los directorios, muestra un resumen del estado de las particiones y los discos, mostrando cuanto almacenamiento ocupa cada directorio

# Monitoreo del Rendimiento

## PerfMon

- Mide, registra y analiza el uso de los recursos del sistema

- SO: **Windows** 

- Interfaz: **Ambas**

- Origen: **Nativa**

- Opciones de lanzamiento esenciales: **perfmon /res** (Abre directamente el monitor de recursos)

- Funcionalidades incorporadas del entorno gráfico: Muestra varias gráficas mostrand ola información de los principales componentes.

::: note
Stacer también encaja parcialmente aquí, ya que incorpora un panel para monitorear CPU, RAM y disco.
:::

# Copias de Seguridad y Gestión de Archivos

## rsync

- Copia y sincroniza archivos local o remotamente

- SO: **Linux** 

- Interfaz: **CLI**

- Origen: **Nativa**

- Opciones de lanzamiento esenciales: **-a** (Modo archivo), **-v** (Modo detallado), **-z** (Comprimir) y **-P** (Muestra una barra de progreso y mantiene los archivosd parcialmente descargados si hay una interrupción)

## Zstd

- Comprime datos sin pérdida

- SO: **Ambos** 

- Interfaz: **CLI**

- Origen: **Nativa**

# Seguridad y Privacidad (Cifrado)

## Cipher

- Mantiene la privacidad y el espacio.

- SO: **Windows** 

- Interfaz: **CLI**

- Origen: **Nativa**
