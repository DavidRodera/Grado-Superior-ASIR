...
function mostrarMenu(){
	...
	echo '4) Reconfigurar un programa instalado (dpkg)'
	echo "=================================================================="
}
...
function operacionextra(){

	echo '=== RECONFIGURAR UN PAQUETE INSTALADO - dpkg ==='
	echo ''
	read -p 'Introduce como crees que se llama el programa que deseas RECONFIGURAR: ' rec_dpkg
	echo ''				
	sudo dpkg -l | grep $rec_dpkg
	echo ''
	read -p 'Introduce el nombre del programa que deseas RECONFIGURAR: ' rec_dpkg_1
	echo ''
	sudo dpkg --configure $rec_dpkg_1 2>/dev/null
	if [ $? -eq 0 ]
	then
		echo ''
		echo 'Programa RECONFIGURADO con ÉXITO ✓'
		echo ''
		read -p 'Pulsa ENTER para volver al Menú Principal...' enter
		return	
	else
		echo 'No hace falta RECONFIGURAR el programa ✓'
		echo ''
		read -p 'Pulsa ENTER para volver al Menú Principal...' enter
		return	
	fi
